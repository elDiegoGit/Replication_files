...
if length(dataq) == length(datam)/3;
        
disp('New quarterly data found! Performing Chow-lin interpolation....');
...
% Create C matrix:
c=zeros(length(dataq),length(datam));
j=zeros(length(dataq))+3;
j(1,1)=0;
k=1;
for i=1:116
...
vhatGA=zeros(length(datam),length(datam));
...
vhatFL=zeros(length(datam),length(datam));
for i=1:length(datam)
for j=1:length(datam)
...
vhatAL=zeros(length(datam),length(datam));
for i=1:length(datam)
for j=1:length(datam)
...
vhatTN=zeros(length(datam),length(datam));
for i=1:length(datam)
for j=1:length(datam)
...
vhatLA=zeros(length(datam),length(datam));
for i=1:length(datam)
for j=1:length(datam)
...
vhatMS=zeros(length(datam),length(datam));
for i=1:length(datam)
for j=1:length(datam)
...
disp('No new quarterly data detected. Performing projection ...');

diff = abs(3*length(dataq) - length(datam));
% Create C matrix:
c=zeros(length(dataq),length(datam)-diff);
j=zeros(length(dataq))+3;
j(1,1)=0;
k=1;
for i=1:116
...
%Unemployment:
UQGA = c*datam(1:length(datam)-diff,1);
UQFL = c*datam(1:length(datam)-diff,4);
UQAL = c*datam(1:length(datam)-diff,7);
UQTN = c*datam(1:length(datam)-diff,10);
UQLA = c*datam(1:length(datam)-diff,13);
UQMS = c*datam(1:length(datam)-diff,16);
% Employment:
EQGA = c*datam(1:length(datam)-diff,2);
EQFL = c*datam(1:length(datam)-diff,5);
EQAL = c*datam(1:length(datam)-diff,8);
EQTN = c*datam(1:length(datam)-diff,11);
EQLA = c*datam(1:length(datam)-diff,14);
EQMS = c*datam(1:length(datam)-diff,17);
% Average hours worked per week:
HQGA = c*datam(1:length(datam)-diff,3);
HQFL = c*datam(1:length(datam)-diff,6);
HQAL = c*datam(1:length(datam)-diff,9);
HQTN = c*datam(1:length(datam)-diff,12);
HQLA = c*datam(1:length(datam)-diff,15);
HQMS = c*datam(1:length(datam)-diff,18);
...
vhatGA=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatGA(i,j)=ahatGA.^abs(i-j);
end
end

vhatFL=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatFL(i,j)=ahatFL.^abs(i-j);
end
...
vhatAL=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatAL(i,j)=ahatAL.^abs(i-j);
end
...
vhatTN=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatTN(i,j)=ahatTN.^abs(i-j);
end
...
vhatLA=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatLA(i,j)=ahatLA.^abs(i-j);
end
...
vhatMS=zeros(length(datam)-diff,length(datam)-diff);
for i=1:length(datam)-diff
for j=1:length(datam)-diff
vhatMS(i,j)=ahatMS.^abs(i-j);
end
...
% Aggregate monthly, observable data per state:
GAM1 = [datam(1:length(datam)-diff,1:3)];
FLM1 = [datam(1:length(datam)-diff,4:6)];
ALM1 = [datam(1:length(datam)-diff,7:9)];
TNM1 = [datam(1:length(datam)-diff,10:12)];
LAM1 = [datam(1:length(datam)-diff,13:15)];
MSM1 = [datam(1:length(datam)-diff,16:18)];


% Chow-lin linear, unbiased estimate of the unobserved montly series per state:
yhatMGA1 = GAM1 * betahatGLSGA + vhatGA*c'*(c*vhatGA*c')^-1*resGLSGA;
yhatMFL1 = FLM1 * betahatGLSFL + vhatFL*c'*(c*vhatFL*c')^-1*resGLSFL;
yhatMAL1 = ALM1 * betahatGLSAL + vhatAL*c'*(c*vhatAL*c')^-1*resGLSAL;
yhatMTN1 = TNM1 * betahatGLSTN + vhatTN*c'*(c*vhatTN*c')^-1*resGLSTN;
yhatMLA1 = LAM1 * betahatGLSLA + vhatLA*c'*(c*vhatLA*c')^-1*resGLSLA;
yhatMMS1 = MSM1 * betahatGLSMS + vhatMS*c'*(c*vhatMS*c')^-1*resGLSMS;

% Projection using previous quarter's estimates:

% Redefine variables:
GAM2 = [datam(length(datam)-diff+1:end,1:3)];
FLM2 = [datam(length(datam)-diff+1:end,4:6)];
ALM2 = [datam(length(datam)-diff+1:end,7:9)];
TNM2 = [datam(length(datam)-diff+1:end,10:12)];
LAM2 = [datam(length(datam)-diff+1:end,13:15)];
MSM2 = [datam(length(datam)-diff+1:end,16:18)];

% Get linear estimate:
yhatMGA2 = GAM2 * betahatGLSGA; %+ vhatGA(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatGA(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSGA(end,1);
yhatMFL2 = FLM2 * betahatGLSFL; %+ vhatFL(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatFL(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSFL(end,1);
yhatMAL2 = ALM2 * betahatGLSAL; %+ vhatAL(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatAL(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSAL(end,1);
yhatMTN2 = TNM2 * betahatGLSTN; %+ vhatTN(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatTN(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSTN(end,1);
yhatMLA2 = LAM2 * betahatGLSLA; %+ vhatLA(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatLA(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSAL(end,1);
yhatMMS2 = MSM2 * betahatGLSMS; %+ vhatMS(length(datam)-diff:length(datam))*c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))'*(c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))* vhatMS(length(datam)-diff:length(datam)) * c(length(dataq)-diff:length(dataq),length(datam)-diff:length(datam))')^-1*resGLSMS(end,1);

% Agregate montly data series:
yhatMGA = [yhatMGA1; yhatMGA2];
yhatMFL = [yhatMFL1; yhatMFL2]; 
yhatMAL = [yhatMAL1; yhatMAL2];
yhatMTN = [yhatMTN1; yhatMTN2]; 
yhatMLA = [yhatMLA1; yhatMLA2]; 
yhatMMS = [yhatMMS1; yhatMMS2];
